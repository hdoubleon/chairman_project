...
export function BuildingDetailScreen({ onNavigate, buildingId }: BuildingDetailScreenProps) {
  const { state, dispatch } = useStudyRoom();
  const [selectedFloorDropdown, setSelectedFloorDropdown] = useState<string | null>(null);
  ...
  const getBuildingImage = (buildingCode: string) => {
    switch (buildingCode) {
