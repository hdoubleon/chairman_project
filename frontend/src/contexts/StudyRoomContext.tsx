...
export interface Floor {
  ...
  name: string;
  seats: Seat[];
  layout: 'library' | 'classroom' | 'study-room';
}
...
export interface StudyRoomState {
  ...
  reservations: { [seatId: string]: { user: User; startTime: Date; endTime: Date } };
}

type StudyRoomAction =
  | { type: 'SET_USER'; payload: User }
  | { type: 'SELECT_SEAT'; payload: string | null }
  | { type: 'SELECT_BUILDING'; payload: string | null }
  | { type: 'SELECT_FLOOR'; payload: string | null }
  | { type: 'RESERVE_SEAT'; payload: { seatId: string; user: User; startTime: Date; endTime: Date } }
  | { type: 'CANCEL_RESERVATION'; payload: string }
  | { type: 'UPDATE_SEAT_STATUS'; payload: { seatId: string; isAvailable: boolean } };

// 미래관 1층 자리
const miraeB1Seats: Seat[] = Array.from({ length: 48 }, (_, i) => ({
...
// 미래관 2층 자리
const miraeB2Seats: Seat[] = Array.from({ length: 36 }, (_, i) => ({
...
// 별관도서관 1층 자리
const libraryB1Seats: Seat[] = Array.from({ length: 60 }, (_, i) => ({
...
// 별관도서관 2층 자리
const libraryB2Seats: Seat[] = Array.from({ length: 45 }, (_, i) => ({
...
// 상상관 1층 자리
const sangsangB1Seats: Seat[] = Array.from({ length: 24 }, (_, i) => ({
...
function studyRoomReducer(state: StudyRoomState, action: StudyRoomAction): StudyRoomState {
  switch (action.type) {
    case 'SET_USER':
...
const StudyRoomContext = createContext<{
  ...
  dispatch: React.Dispatch<StudyRoomAction>;
} | null>(null);
